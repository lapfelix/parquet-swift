// Int32ColumnReader - Read Int32 column values
//
// Licensed under the Apache License, Version 2.0

import Foundation

/// Reads Int32 values from a Parquet column chunk.
///
...
/// )
///
/// let values = try reader.readBatch(count: 1000)  // Returns [Int32?]
/// ```
public final class Int32ColumnReader {
    ...
    private let dictionary: Dictionary<Int32>?
    ...
    private var currentDecoder: PlainDecoder<Int32>?
    ...
    /// Initialize an Int32 column reader
    ///
    /// - Parameters:
    ...
    public init(
        ...
        if let dictPage = try pageReader.readDictionaryPage() {
            self.dictionary = try Dictionary.int32(page: dictPage)
        } else {
            self.dictionary = nil
        }
    }
    ...
    // MARK: - Internal API for Struct and Map Reading
    ...
    ///
    /// This method walks through all pages once, collecting values (with nil for NULLs),
    /// definition levels, and repetition levels in parallel.
    ///
    /// - Returns: Tuple of (values, definition levels, repetition levels)
    /// - Throws: `ColumnReaderError` if reading fails
    internal func readAllWithAllLevels() throws -> (values: [Int32?], definitionLevels: [UInt16], repetitionLevels: [UInt16]) {
        var values: [Int32?] = []
        var defLevels: [UInt16] = []
        ...
        while try loadPageIfNeeded() {
            ...
            for _ in 0..<numValuesInPage {
                ...
                let repLevel: UInt16
                if let currentRepLevels = currentRepetitionLevels {
                    repLevel = currentRepLevels[valuesReadFromPage]
                } else {
                    // No repetition levels means non-repeated column
                    repLevel = 0
                }
    ...
    public func readAllRepeated() throws -> [[Int32?]?] {
        ...
        var allValues: [Int32] = []  // Non-null values only
    ...
    private func loadPageIfNeeded() throws -> Bool {
        ...
        case .plain:
            ...
            let decoder = PlainDecoder<Int32>(data: dataSlice)
            currentPage = page
        ...
        default:
            throw ColumnReaderError.unsupportedEncoding(
                "Unsupported encoding \(page.encoding) for Int32 column"
            )
        }
