...
public final class FloatColumnReader {
    ...
    internal func readAllWithLevels() throws -> (values: [Float?], definitionLevels: [UInt16]) {
        var values: [Float?] = []
    ...
    internal func readAllWithAllLevels() throws -> (values: [Float?], definitionLevels: [UInt16], repetitionLevels: [UInt16]) {
        var values: [Float?] = []
        var defLevels: [UInt16] = []
    ...
    /// # Example
    ///
    /// For schema: `repeated float numbers;`
    /// Data: [[1.0, 2.0], None, [], [3.0]]
    /// Returns: [[1.0, 2.0], nil, [], [3.0]]
    public func readAllRepeated() throws -> [[Float?]?] {
        guard maxRepetitionLevel > 0 else {
            throw ColumnReaderError.unsupportedFeature(
        ...
        var allValues: [Float] = []  // Non-null values only
    ...
    ///   - maxRepLevel=2: `[[[Float?]?]?]` (list of optional lists of optional values)
    ///   - maxRepLevel=3: `[[[[Float?]?]?]?]` (and so on)
    ///
    ...
    ///
    /// ```swift
    /// let url = URL(fileURLWithPath: "data.parquet")
    ...
    /// let typedResult = result as! [[[Float?]?]?]
    /// // typedResult contains lists of lists of optional Float values
    /// ```
